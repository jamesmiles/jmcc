// TEST: rosetta_stable_marriage
// DESCRIPTION: Rosetta Code - Stable marriage problem (enums, 2D arrays, complex logic)
// EXPECTED_EXIT: 0
// ENVIRONMENT: hosted
// PHASE: 5
// SOURCE: https://rosettacode.org/wiki/Stable_marriage_problem#C
// LICENSE: GFDL 1.2
// EXPECTED_STDOUT:
// STDOUT: Pairing:
// STDOUT:    Abe - Ivy
...
// STDOUT:    Ian - Hope
// STDOUT:    Jon - Abi
// STDOUT: Stable matchup
// STDOUT:
// STDOUT: But if Bob and Fred were to swap:
// STDOUT:     Fred (w/ Cath) and  Ivy (w/  Abe) prefer each other over current pairing.
// STDOUT:      Bob (w/  Bea) and  Fay (w/  Dan) prefer each other over current pairing.
// STDOUT:      Bob (w/  Bea) and Hope (w/  Ian) prefer each other over current pairing.
// STDOUT:      Bob (w/  Bea) and  Abi (w/  Jon) prefer each other over current pairing.
// STDOUT:     Fred (w/ Cath) and  Dee (w/  Col) prefer each other over current pairing.
// STDOUT:     Fred (w/ Cath) and  Abi (w/  Jon) prefer each other over current pairing.
// STDOUT: Marriages not stable
